const run = (dirname = null) => {
	...
	fs.writeFileSync(dirname + '/routes/web.js', fs.readFileSync('./src/routes/web.js').toString());

	// Create a sample "api.js" file inside "routes"
	fs.writeFileSync(dirname + '/routes/api.js', fs.readFileSync('./src/routes/api.js').toString());

	// Create entrypoint for a whole app
	fs.writeFileSync(dirname + '/index.js', fs.readFileSync('./src/index.js').toString());

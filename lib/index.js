const run = (dirname = null) => {
	...
	if (args.includes('--version') || args.includes('-v')) {
		return console.log(`Version ${packageJson.version}`)
	}
	...
	let execute = exec(`cd ${dirname} && npm install --save express sequelize sequelize-cli pg pg-hstore mysql2 express-validator multer app-module-path dotenv morgan compression cors && npx sequelize-cli init --models-path app/Models --migrations-path database/migrations --seeders-path database/seeders && npm install -g nodemon`)

	execute.stdout.on('data', (data) => {
		console.log(data);
	})

	fs.writeFileSync(dirname + '/config/database.js', fs.readFileSync(path.join(__dirname, 'src/config/database.js')).toString());
	fs.writeFileSync(dirname + '/.env', fs.readFileSync(path.join(__dirname, '/.env.example')).toString());
